...
const valueProps = [
  ...
  { icon: FlaskConical, key: "vp2" },
  { icon: Zap, key: "vp3" },
  { icon: Clock, key: "vp4" },
] as const;

export function EntrepotLanding() {
  const t = useTranslations("lpEntrepot");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/quote", {
        ...
        body: JSON.stringify({ ...formData, source: "lp-entrepot", projectType: "warehouse" }),
      });
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          ...
          <div
            ...
            style={{ background: "radial-gradient(circle, #d97706 0%, transparent 70%)", filter: "blur(60px)" }}
          />
        </div>
              ...
              <motion.div variants={blurUp} className="mt-8 grid grid-cols-3 gap-4">
                <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm p-3">
                  <div className="text-2xl font-bold font-[family-name:var(--font-cabinet)]">
                    <NumberTicker value={50} suffix="K+" delay={0.2} />
                  </div>
                  <div className="text-xs text-muted/60">{t("statSqft")}</div>
                </div>
                <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm p-3">
                  <div className="text-2xl font-bold font-[family-name:var(--font-cabinet)]">
                ...
                <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm p-3">
                  <div className="text-2xl font-bold font-[family-name:var(--font-cabinet)]">48h</div>
                  <div className="text-xs text-muted/60">{t("statInstall")}</div>
                </div>
              </motion.div>
