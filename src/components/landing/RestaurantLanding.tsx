...
const valueProps = [
  ...
  { icon: Footprints, key: "vp3" },
  { icon: ClipboardCheck, key: "vp4" },
] as const;

export function RestaurantLanding() {
  const t = useTranslations("lpRestaurant");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/quote", {
        ...
        body: JSON.stringify({ ...formData, source: "lp-restaurant", projectType: "restaurant" }),
      });
            ...
            <motion.div variants={stagger} initial="hidden" animate="visible">
              ...
              <motion.div variants={blurUp} className="mt-8 grid grid-cols-3 gap-4">
                <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm p-3">
                  ...
                  <div className="text-xs text-muted/60">{t("statFoodSafe")}</div>
                </div>
                <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm p-3">
                  <div className="text-2xl font-bold font-[family-name:var(--font-cabinet)]">
