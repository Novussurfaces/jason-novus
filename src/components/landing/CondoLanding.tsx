...
const valueProps = [
  { icon: Car, key: "vp1" },
  { icon: Paintbrush, key: "vp2" },
  { icon: TrendingUp, key: "vp3" },
  { icon: ShieldCheck, key: "vp4" },
] as const;

export function CondoLanding() {
  const t = useTranslations("lpCondo");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/quote", {
        ...
        body: JSON.stringify({ ...formData, source: "lp-condo", projectType: "condo" }),
      });
              ...
              <motion.div variants={blurUp} className="mt-8 grid grid-cols-3 gap-4">
                <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm p-3">
                  <div className="text-2xl font-bold font-[family-name:var(--font-cabinet)]">
                    <NumberTicker value={200} suffix="+" delay={0.2} />
                  </div>
                  <div className="text-xs text-muted/60">{t("statCondos")}</div>
                </div>
                <div className="text-center rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm p-3">
                  <div className="text-2xl font-bold font-[family-name:var(--font-cabinet)]">
